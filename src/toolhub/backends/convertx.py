...
class ConvertXClient:
    ...
    def _client(self) -> httpx.AsyncClient:
        return httpx.AsyncClient(
            base_url=self.settings.convertx_base_url.rstrip("/"),
    ...
    async def health(self) -> dict[str, str | bool]:
        try:
            ...
            return {
                "reachable": False,
                "base_url": self.settings.convertx_base_url,
                "error": str(exc),
            }
        return {"reachable": reachable, "base_url": self.settings.convertx_base_url}
